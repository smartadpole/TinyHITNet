    ...
    while True:
        for ids, lock in enumerate(process.lock_list):
            if lock.acquire(block=False):
                disp_path = (process.root / "scale_tof_crop_disp" / file_path).with_suffix(
                    ".png"
                )
                dxy_path = (process.root / "scale_tof_crop_disp_slant_window" / file_path).with_suffix(
                    ".npy"
                )
...
if __name__ == "__main__":
    main("/data/Parker/DEPTH/CREStereo_100_tof/TRAIN/data_2023_08_23", "../lists/indemind359.list")
